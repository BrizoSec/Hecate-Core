...
class SigmaGeneratorAgent(LLMAgent[SigmaGenerationInput, SigmaGenerationOutput]):
    ...
    def execute(self, input_data: SigmaGenerationInput) -> AgentResult[SigmaGenerationOutput]:
        ...
        try:
            prompt = self._build_prompt(input_data, logsources)
            ...
            output_text = self._call_llm_with_retry(prompt, validate_json, max_retries=2, temperature=0.1)
            payload = self._parse_json_response(output_text)
            rules, rejected = self._collect_rules(payload.get("rules", []), input_data)

            if not rules:
                raise ValueError("every generated rule failed validation: " + "; ".join(rejected))

            covered = {r.logsource_category for r in rules}
            for missing in sorted(set(logsources) - covered):
                warnings.append(f"no valid rule generated for log source '{missing}'")
            ...
            return AgentResult(
                success=True,
                data=SigmaGenerationOutput(
                    rules=rules,
                    logsources_considered=logsources,
                    rejected=rejected,
                ),
                error=None,
                warnings=warnings,
    ...
    def _build_prompt(self, input_data: SigmaGenerationInput, logsources: List[str]) -> str:
        ...
        return f"""You are a detection engineer drafting Sigma rules for a threat hunt.
...
Write ONE rule for each of these Sigma log source categories:
{", ".join(logsources)}

RULES OF THE TASK:
1. Return JSON only. Do NOT write YAML - the caller renders it.
2. Use field names real telemetry actually has for that log source, e.g.
   process_creation: Image, CommandLine, ParentImage, OriginalFileName
   network_connection: DestinationIp, DestinationHostname, DestinationPort, Initiated
   dns_query: QueryName, QueryResults
   file_event: TargetFilename, Image
   registry_event: TargetObject, Details
   image_load: ImageLoaded, Image, Signed
  ...
  "rules": [
    {{
      ...
      "detection": {{
        ...
        "filter": {{"CommandLine|contains": "known-good"}},
