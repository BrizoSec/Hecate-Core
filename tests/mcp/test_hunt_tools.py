...
class TestHuntList:
    def test_lists_all_hunts(self, server):
        result = _call_tool(server, "hecate_hunt_list")
        assert result["count"] == 2

    def test_filter_by_status(self, server):
        result = _call_tool(server, "hecate_hunt_list", {"status": "completed"})
        assert result["count"] == 1
        assert result["hunts"][0]["hunt_id"] == "H-0001"

    def test_filter_by_tactic(self, server):
        result = _call_tool(server, "hecate_hunt_list", {"tactic": "lateral-movement"})
        assert result["count"] == 1
...
class TestHuntSearch:
    def test_search_finds_match(self, server):
        result = _call_tool(server, "hecate_hunt_search", {"query": "Kerberoasting"})
        assert result["count"] >= 1

    def test_search_no_match(self, server):
        result = _call_tool(server, "hecate_hunt_search", {"query": "zzz_nonexistent_zzz"})
        assert result["count"] == 0


class TestHuntGet:
    def test_get_existing_hunt(self, server):
        result = _call_tool(server, "hecate_hunt_get", {"hunt_id": "H-0001"})
        assert result["hunt_id"] == "H-0001"
        assert "frontmatter" in result

    def test_get_nonexistent_hunt(self, server):
        result = _call_tool(server, "hecate_hunt_get", {"hunt_id": "H-9999"})
        assert "error" in result


class TestHuntStats:
    def test_returns_stats(self, server):
        result = _call_tool(server, "hecate_hunt_stats")
...
class TestHuntCoverage:
    def test_returns_coverage(self, server):
        result = _call_tool(server, "hecate_hunt_coverage")
    ...
    def test_filter_by_tactic(self, server):
        result = _call_tool(server, "hecate_hunt_coverage", {"tactic": "credential-access"})
...
class TestHuntValidate:
    def test_validate_valid_hunt(self, server):
        result = _call_tool(server, "hecate_hunt_validate", {"hunt_id": "H-0001"})
        assert result["valid"] is True

    def test_validate_nonexistent(self, server):
        result = _call_tool(server, "hecate_hunt_validate", {"hunt_id": "H-9999"})
