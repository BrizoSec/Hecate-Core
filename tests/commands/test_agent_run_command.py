"""Coverage for hecate_agent.commands.agent's `list`/`info` commands, the `run`
...
# hecate_agent/commands/__init__.py does `from hecate_agent.commands.agent import agent`,
# shadowing the submodule reference the same way it does for `research` --
# see the note in test_research.py.
agent_cmd = importlib.import_module("hecate_agent.commands.agent")
...
class TestRunHypothesisGenerator:
    ...
    def test_success_displays_result(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.HypothesisGeneratorAgent", lambda llm_enabled: _fake_hypothesis_agent())
    ...
    def test_json_output_format(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        ...
        result = runner.invoke(agent, ["run", "hypothesis-generator", "--threat-intel", "APT29", "--output-format", "json"])
    ...
    def test_agent_execution_error_aborts(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.HypothesisGeneratorAgent", _raise)
    ...
    def test_research_option_loads_context_when_found(
        ...
        monkeypatch.setattr(
            "hecate_agent.core.research_manager.ResearchManager.extract_research_context",
            lambda self, doc: {"topic": "fake"},
        )

        result = runner.invoke(agent, ["run", "hypothesis-generator", "--threat-intel", "APT29", "--research", "R-0001"])

        assert "Loaded research context from R-0001" in result.output

    def test_research_option_warns_when_not_found(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        monkeypatch.chdir(tmp_path)
        monkeypatch.setattr("hecate_agent.agents.llm.HypothesisGeneratorAgent", lambda llm_enabled: _fake_hypothesis_agent())
        monkeypatch.setattr("hecate_agent.core.research_manager.ResearchManager.get_research", lambda self, research_id: None)

        result = runner.invoke(agent, ["run", "hypothesis-generator", "--threat-intel", "APT29", "--research", "R-9999"])
    ...
    def test_technique_option_auto_discovers_research(
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.HypothesisGeneratorAgent", lambda llm_enabled: _fake_hypothesis_agent())
        monkeypatch.setattr(
            "hecate_agent.core.research_manager.ResearchManager.find_by_technique",
        ...
        result = runner.invoke(agent, ["run", "hypothesis-generator", "--threat-intel", "APT29", "--technique", "T1003.001"])
    ...
    def test_import_error_shows_install_hints(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        ...
        assert "pip install 'hecate-agent[litellm]'" in result.output

    def test_environment_md_loaded_when_present(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
        monkeypatch.chdir(tmp_path)
        captured = {}
        ...
        def _capturing_execute(self, input_data):
            ...
            return AgentResult(
                success=True, data=_fake_hypothesis_agent().execute(None).data, error=None, warnings=[], metadata={}
            )
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.HypothesisGeneratorAgent", HypothesisGeneratorAgent)
...
class TestRunHuntResearcher:
    ...
    def test_success_displays_result(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = type(
            ...
            (),
            {
                "execute": lambda self, input_data: AgentResult(
                    success=True, data=_make_research_output(), error=None, warnings=[], metadata={}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.hunt_researcher.HuntResearcherAgent", lambda llm_enabled: fake)
    ...
    def test_failure_aborts(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = type(
            ...
            {
                "execute": lambda self, input_data: AgentResult(
                    success=False, data=None, error="search timed out", warnings=[], metadata={}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.hunt_researcher.HuntResearcherAgent", lambda llm_enabled: fake)
    ...
    def test_import_error_shows_install_hints(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.hunt_researcher.HuntResearcherAgent", _raise)
    ...
    def test_json_output_format(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = type(
            ...
            {
                "execute": lambda self, input_data: AgentResult(
                    success=True, data=_make_research_output(), error=None, warnings=[], metadata={"x": 1}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.hunt_researcher.HuntResearcherAgent", lambda llm_enabled: fake)
        ...
        # hecate-runner's hecate_client.py has to parse around for `research
        # new`.
...
class TestRunPivotSuggester:
    ...
    def test_success_displays_result(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        data = PivotOutput(
            ...
            pivots=[
                PivotSuggestion(
                    query="search parent=powershell.exe",
                    rationale="check origin",
                    data_source="EDR",
                    priority=1,
        ...
        fake = type(
            ...
            {
                "execute": lambda self, input_data: AgentResult(
                    success=True, data=data, error=None, warnings=[], metadata={"mode": "llm"}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.pivot_suggester.PivotSuggesterAgent", lambda llm_enabled: fake)
    ...
    def test_heuristic_mode_shows_note(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        fake = type(
            ...
            {
                "execute": lambda self, input_data: AgentResult(
                    success=True, data=data, error=None, warnings=[], metadata={"mode": "heuristic"}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.pivot_suggester.PivotSuggesterAgent", lambda llm_enabled: fake)
    ...
    def test_failure_aborts(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        fake = type(
            ...
            {
                "execute": lambda self, input_data: AgentResult(
                    success=False, data=None, error="LLM error", warnings=[], metadata={}
                )
            },
        )()
        monkeypatch.setattr("hecate_agent.agents.llm.pivot_suggester.PivotSuggesterAgent", lambda llm_enabled: fake)
    ...
    def test_import_error_shows_install_hints(self, runner: CliRunner, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("hecate_agent.agents.llm.pivot_suggester.PivotSuggesterAgent", _raise)
