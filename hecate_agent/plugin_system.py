"""Plugin system for Hecate extensions."""
...
class PluginRegistry:
    """Central registry for Hecate plugins."""
    ...
    def load_plugins(cls) -> None:
        ...
        try:
            ...
            if sys.version_info >= (3, 10):
                eps = entry_points(group="hecate_agent.commands")
            else:
                eps = entry_points().get("hecate_agent.commands", [])
        ...
        try:
            # Python 3.10+ uses group= parameter, 3.8-3.9 uses dict-like access
            if sys.version_info >= (3, 10):
                eps = entry_points(group="hecate_agent.agents")
            else:
                eps = entry_points().get("hecate_agent.agents", [])
