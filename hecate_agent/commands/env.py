...
Examples:
  ...
  hecate-agent env setup --python python3.13
...
def env() -> None:
    ...
    virtual environment for Hecate development.
    """
    pass
...
def setup(python: str, dev: bool, clean: bool) -> None:  # noqa: C901
    ...
    Creates .venv directory and installs the hecate-agent package with
    all dependencies from pyproject.toml.

    \b
    Steps:
    1. Create .venv directory (or clean existing)
    2. Install hecate-agent package in editable mode
    ...
    # It only makes sense when run from the Hecate source repository root.
    # Users who installed Hecate via pip already have a working environment
    # and do not need to run this command.
    if not Path("pyproject.toml").exists():
        console.print("[red]Error: pyproject.toml not found in the current directory.[/red]")
        console.print()
        console.print("[bold]hecate-agent env setup[/bold] is for Hecate source-repo developers only.")
        console.print("It creates a local .venv by running [cyan]pip install -e .[/cyan] from the repo root.")
        console.print()
        console.print("If you installed Hecate via pip, your environment is already set up —")
        console.print("you can run [cyan]hecate-agent[/cyan] commands directly without a separate venv.")
        raise click.Abort()
    ...
    if venv_path.exists():
        ...
        usage_panel = Panel(
            ...
            f"   {activate_cmd}\n\n"
            f"[green]2. Run hecate-agent commands:[/green]\n"
            f"   hecate-agent --version\n"
            f"   hecate-agent hunt --help\n\n"
            f"[green]3. Or use without activating:[/green]\n"
            f"   .venv/bin/hecate-agent [command]\n\n"
    ...
    try:
        ...
        console.print("[dim]Try: hecate-agent env setup --python python3.13[/dim]")
        raise click.Abort()
    ...
    # Install hecate-agent package
    console.print("[cyan]📦 Installing Hecate package...[/cyan]")
    ...
    try:
        ...
        console.print("[green]✅ Hecate installed[/green]\n")
    except subprocess.CalledProcessError as e:
        console.print(f"[red]Error installing package: {e.stderr}[/red]")
        raise click.Abort()

    # Install scikit-learn for hecate-agent similar command
    ...
    try:
        ...
        console.print("[dim]hecate-agent similar command will not work without scikit-learn[/dim]\n")
    ...
    activation_panel = Panel(
        ...
        f"[white]hecate-agent --version[/white]\n"
        f"[white]hecate-agent hunt --help[/white]",
        title="🚀 Next Steps",
        border_style="green",
    )
...
def clean() -> None:
    ...
    try:
        ...
        console.print("[dim]Run 'hecate-agent env setup' to recreate[/dim]")
    except Exception as e:
        console.print(f"[red]Error removing .venv: {e}[/red]")
        raise click.Abort()
...
def info() -> None:  # noqa: C901
    ...
    if not venv_path.exists():
        console.print("[yellow]No .venv directory found[/yellow]")
        console.print("[dim]Run 'hecate-agent env setup' to create[/dim]")
        return
    ...
    if not python_path.exists():
        ...
        console.print("[dim]Run 'hecate-agent env setup --clean' to recreate[/dim]")
        return
    ...
    # Check for hecate-agent installation
    try:
        result = subprocess.run(
        ...
        hecate_installed = "✅ Installed" if result.returncode == 0 else "❌ Not installed"
    except subprocess.CalledProcessError:
        hecate_installed = "❌ Not installed"
    ...
    console.print(f"  [cyan]hecate:[/cyan] {hecate_installed}")
    console.print(f"  [cyan]scikit-learn:[/cyan] {sklearn_installed} [dim](required for hecate-agent similar)[/dim]")
    console.print()
...
def check() -> None:  # noqa: C901
    """Run a dependency health checklist for your Hecate workspace.
    ...
    py_ok = ver >= (3, 9)
    _check(f"Python {ver.major}.{ver.minor}.{ver.micro}", py_ok, "" if py_ok else "requires Python 3.9+")

    # hecate-agent package
    try:
        import hecate_agent

        package_version = getattr(hecate_agent, "__version__", "unknown")
        _check(f"hecate-agent {package_version}", True)
    except ImportError:
        _check("hecate-agent", False, "not importable — installation may be broken")

    # scikit-learn
    sklearn_ok = importlib.util.find_spec("sklearn") is not None
    _check("scikit-learn", sklearn_ok, "required for `hecate-agent similar`" if not sklearn_ok else "", optional=True)

    # litellm
    litellm_ok = importlib.util.find_spec("litellm") is not None
    _check("litellm", litellm_ok, "required for AI agents (`hecate-agent agent run`)" if not litellm_ok else "", optional=True)
    ...
    _check(
        "mitreattack-python",
        mitreattack_ok,
        "required for STIX technique data (`hecate-agent attack update`)" if not mitreattack_ok else "",
        optional=True,
    )
    ...
    if mitreattack_ok:
        try:
            from hecate_agent.core.attack_matrix import _resolve_stix_file, is_using_stix
            ...
            _check(
                "ATT&CK STIX data",
                stix_ok,
                str(stix_path) if stix_ok else f"not found at {stix_path} — run `hecate-agent attack update`",
                optional=True,
            )
        except Exception:
    ...
    _check("tavily-python", tavily_ok, "required for web search in agents" if not tavily_ok else "", optional=True)

    # Workspace config
    from hecate_agent.commands._hunt_create import get_config_path

    config_path = get_config_path()
    config_ok = config_path.exists()
    _check(".hecateconfig.yaml", config_ok, str(config_path) if config_ok else "not found")
