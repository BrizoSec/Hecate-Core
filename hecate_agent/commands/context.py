...
Examples:
  ...
  hecate-agent context --hunt H-0013

  # Export context for all credential access hunts
  hecate-agent context --tactic credential-access

  # Export context for macOS platform hunts
  hecate-agent context --platform macos

  # Combine filters: persistence hunts on Linux
  hecate-agent context --tactic persistence --platform linux

  # Export full repository context (large output)
  hecate-agent context --full
...
def context(
    ...
    if not has_filter:
        ...
        console.print("\n[dim]Examples:[/dim]")
        console.print("  hecate-agent context --hunt H-0013")
        console.print("  hecate-agent context --tactic credential-access")
        console.print("  hecate-agent context --platform macos")
        console.print("  hecate-agent context --tactic persistence --platform linux")
        raise click.Abort()
