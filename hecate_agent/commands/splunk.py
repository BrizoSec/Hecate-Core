"""Splunk integration commands for Hecate.
...
def splunk() -> None:
    ...
    \b
    Execute SPL queries and interact with Splunk directly from Hecate CLI.
    ...
    Examples:
      ...
      # Execute a query
      hecate-agent splunk search 'index=main "Failed password" | head 10'

      # Query with time range
      hecate-agent splunk search 'index=* | stats count by sourcetype' \\
        --earliest "-7d" --latest "now" --count 100
    """


def _render_results(results: list, output_format: str, count: int) -> None:
    """Print search results in the requested shape.

    Split out of `search` because format selection has nothing to do with
    running the query, and the three branches together were most of that
    command's complexity.
    """
    ...
    # Table. Splunk events are heterogeneous, so the columns are the union of
    # every field seen rather than the first event's keys.
    all_fields = sorted({field for result in results for field in result})
    table = Table(title=f"Search Results ({len(results)} events)")
    for field in all_fields:
        table.add_column(field, overflow="fold")
    for result in results[:count]:
        table.add_row(*[str(result.get(field, "")) for field in all_fields])
    console.print(table)
...
def search(
    ...
    Examples:
      # Basic search
      hecate-agent splunk search 'index=main error'
      ...
      hecate-agent splunk search 'index=main | head 10' --format json

      # Long-running query (async)
      hecate-agent splunk search 'index=* | rare limit=20 sourcetype' \\
    ...
    try:
        ...
        _render_results(results, output_format, count)
