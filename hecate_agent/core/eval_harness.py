...
from hecate_agent.core.llm_provider import LLMProvider
...
def build_grounded_fixtures(fixtures: Optional[List[Fixture]] = None) -> List[Fixture]:
    ...
    description (via ``hecate-agent attack lookup``'s data source, no LLM involved)
    ...
    questions that "hecate-agent eval" collapses into one score: (1) does the model
