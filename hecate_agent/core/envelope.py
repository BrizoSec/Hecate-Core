...
def build_envelope(
    ...
    *,
    ...
    if parent_artifact is not None:
        if not path:
            raise ValueError("build_envelope: parent_artifact requires an explicit path argument")
    ...
    if not persist_dir or not artifact_name:
        raise ValueError(
            "build_envelope: Gate B fired (byte_count > threshold) but " "persist_dir and artifact_name were not both supplied"
        )
    ...
    if artifact_rel.is_absolute() or artifact_name.startswith(("/", "\\")):
        raise ValueError("build_envelope: artifact_name must be a relative filename")
    ...
    try:
        ...
        raise ValueError("build_envelope: artifact_name escapes persist_dir") from None
