...
class HuntParser:
    ...
    def _attack_errors(self) -> List[str]:
        """Frontmatter techniques and tactics that the ATT&CK matrix rejects.

        Techniques are only checked when STIX data is present: the fallback
        provider carries no per-technique data, so validating against it would
    ...
    def validate(self) -> Tuple[bool, List[str]]:
        ...
        if hunt_id and filename_stem and hunt_id != filename_stem:
            errors.append(f"hunt_id '{hunt_id}' does not match filename '{filename_stem}.md'")

        errors.extend(self._attack_errors())
