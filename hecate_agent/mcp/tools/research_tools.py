...
def register_research_tools(mcp: "FastMCP") -> None:  # type: ignore[name-defined]  # noqa: F821, C901
    """Register all research-related MCP tools."""

    @mcp.tool(
        name="hecate_research_list",
    ...
    @mcp.tool(
        name="hecate_research_new",
        description=(
            "Run deep research on a security topic using Tavily web search "
            "and the Hecate 5-skill methodology (system internals, adversary tradecraft, "
        ...
        return _json_result(
            {
                ...
                "next_steps": [
                    f"Review full research: hecate-agent research view {output.research_id}",
                    f"Create hunt: hecate-agent hunt new --research {output.research_id}",
                    "Generate hypothesis: hecate-agent agent run hypothesis-generator",
                ],
            }
        )
